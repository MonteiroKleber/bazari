...
// V-5 (2025-09-18): Adiciona itens relacionados, SEO e JSON-LD na PDP de produto
...
export function ProductDetailPage() {
  ...
  const { status, data, error } = useProductDetail(itemId);
  ...
  const related = useRelatedItems(categoryPath, 'product');

  const filteredRelatedItems = useMemo(() => {
    if (!data) return [];
    return related.items.filter(item => item.kind === 'product' && item.id !== data.id);
  ...
  useEffect(() => {
    ...
    const scriptId = `jsonld-product-${data.id}`;
    ...
    if (script) {
      script.textContent = JSON.stringify(generateProductSchema(data));
    }
  ...
  return (
    <>
      <DynamicHeader />
      <main className="pt-16">
        <section className="container mx-auto px-4 py-2 md:py-3 mobile-safe-bottom" aria-labelledby="product-title">
        ...
        {hasRelatedItems ? (
          <RelatedItems items={filteredRelatedItems} kind="product" />
        ) : null}
        </section>
      </main>
      <Footer />
