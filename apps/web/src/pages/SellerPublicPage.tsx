...
export default function SellerPublicPage() {
  ...
  if (error) return <div className="container mx-auto px-4 py-8 text-destructive">{error}</div>;
  if (!data) return null;
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <div className="mt-2 flex items-center gap-3 text-sm text-muted-foreground">
          ...
          {owner?.handle && (
            <Link className="text-primary underline underline-offset-4" to={`/u/${owner.handle}`}>{t('seller.public.ownerProfile', { defaultValue: 'Ver perfil do dono' })}</Link>
          )}
        </div>
      </div>
      ...
      {nextCursor && (
        <div className="mt-6 flex justify-center">
          <button className="px-4 py-2 border rounded" onClick={() => load(nextCursor)}>{t('profile.seeMore')}</button>
        </div>
      )}
    </div>
  );
