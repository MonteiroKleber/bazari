...
import { Plus, Package, Star, Download, DollarSign } from 'lucide-react';
...
export default function DevPortalDashboardPage() {
    ...
    const fetchData = async () => {
      try {
        const [appsRes, revenueRes] = await Promise.all([
          api.get<{ apps: DeveloperApp[] }>('/developer/apps'),
          api.get<{ summary: RevenueSummary | null }>('/developer/revenue/summary').catch(() => ({ summary: null })),
        ]);
        if (active) {
  ...
  return (
    <DeveloperLayout
      ...
      description="Gerencie seus apps no ecossistema Bazari"
      actions={
        <Button asChild>
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => window.location.href = '/app/developer/revenue'}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {(revenue?.last30DaysRevenue || 0).toFixed(2)} BZR
            </div>
            <p className="text-xs text-muted-foreground">
              Total: {(revenue?.developerRevenue || 0).toFixed(2)} BZR
            </p>
          </CardContent>
        </Card>
      </div>
