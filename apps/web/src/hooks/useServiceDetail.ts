// V-3 (2025-09-18): Normaliza mídia do serviço em mediaNormalized com fallback seguro
// V-2 (2025-09-18): Enriquecimento da PDP de serviço (seller, rating, categoria)
...
export function useServiceDetail(id?: string) {
  const [state, setState] = useState<DetailState<ServiceDetail>>({
    ...
    apiHelpers
      ...
      .then((data: any) => {
        ...
        const normalized: ServiceDetail = {
          id: data.id,
          daoId: data.daoId,
          daoName: data.daoName ?? data.providerName ?? null,
          title: data.title,
          description: data.description,
          basePriceBzr: data.basePriceBzr,
